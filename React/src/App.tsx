...
import Chart, { ArgumentAxis, Label, Series } from 'devextreme-react/chart';
...
function App(): JSX.Element {
  ...
  const [chartData, setChartData] = useState<Order[]>(orders);

  const onOptionChanged = useCallback((e: DataGridTypes.OptionChangedEvent): void => {
    if (e.fullName.includes('filterValue')) {
      setTimeout(() => {
        const filterExpr = e.component.getCombinedFilter(true);
        let filteredData = orders;

        if (filterExpr) {
          filteredData = query(orders).filter(filterExpr).toArray();
        }
  ...
  return (
    <div id="app-container">
      ...
      >
        ...
        <Series type="bar" argumentField="OrderDate" valueField="SaleAmount">
          <Label visible={true} />
        </Series>
      </Chart>
    </div>
